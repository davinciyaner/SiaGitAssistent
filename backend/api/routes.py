...
def ci_full_auto_fix(
    ...
    """
    Fixiert exakt einen bestimmten Workflow Run
    """
    ...
    if not os.path.exists(repo_local_path):
        raise HTTPException(
            status_code=400,
            detail=f"repo_local_path '{repo_local_path}' existiert nicht.",
        )
    ...
    if py_files:
        ...
        subprocess.run(["isort"] + py_files, check=False)

        # Lint Check
        subprocess.run(["flake8"] + py_files, check=False)
        ...
        if status.stdout.strip():
            ...
            commit_msg = f"Auto Fix für CI Run {run_id}"
    ...
    # ------------------------------------------------
    # 7️⃣ Workflow erneut starten
    # ------------------------------------------------
    dispatch_url = f"https://api.github.com/repos/{repo_full_name}/actions/workflows/{workflow_id}/dispatches"
    ...
    if dispatch_resp.status_code in (201, 204):
        workflow_dispatch_msg = "Workflow erfolgreich erneut gestartet"
    else:
        workflow_dispatch_msg = f"Workflow Restart nicht möglich: {dispatch_resp.text}"
